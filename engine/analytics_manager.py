...
class AnalyticsManager:
    ...
    @staticmethod
    async def get_age_weighted_top_videos_this_week(session, limit=10):
        ...
        view_subq = (
            select(
                ...
                func.sum(
                    case((user_age <= 5, 1.5), (user_age <= 12, 1.2), else_=1.0)
                ).label("view_score")
            )
        ...
        like_subq = (
            select(
                ...
                func.sum(
                    case((user_age <= 5, 3.0), (user_age <= 12, 2.5), else_=2.0)
                ).label("like_score")
            )
        ...
        save_subq = (
            select(
                ...
                func.sum(
                    case((user_age <= 5, 6.0), (user_age <= 12, 5.0), else_=4.0)
                ).label("save_score")
            )
    ...
    @staticmethod
    async def get_age_weighted_top_stories_this_week(session, limit=10):
        now = datetime.datetime.now()
        start_of_week = now - datetime.timedelta(days=7)
    ...
    @staticmethod
    async def recommend_videos_for_user(user_id: int, session, limit=10):
        ...
        view_subq = (
            ...
            .join(User, User.id == UserView.user_id)
            .where(
                UserView.created_at >= last_week,
                user_age_group_case == user_group
            )
            .group_by(UserView.short_video_id)
            .subquery()
        )
